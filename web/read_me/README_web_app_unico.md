...
- Formulário de cliente: `web/documentation/cadastro-de-cliente.html` → aba `cliente_db_master` (ou `clientes_db_master`).
- Formulário de briefing: `web/documentation/cadastro-briefing.html` → aba `jobs_db`.
- Log público de jobs: `?action=jobs`.
- Roadmap interno/cliente (`web/control/admin_roadmap.html` / `web/control/roadmap_public.html`): abas `roadpmaps` e `roadpmaps_posts`.
...
- Para o cliente: `control/roadmap_public.html?client_slug=<slug-ou-nome>&month=YYYY-MM`.
