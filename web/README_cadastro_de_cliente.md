...
Um único Apps Script recebe os envios dos formulários `web/cadastro-de-cliente.html` e `web/cadastro-briefing.html` e grava tudo na mesma planilha, em abas separadas: `clientes_db_master` e `jobs_db`.

## Como publicar o Web App

1. Abra a planilha que terá as abas `clientes_db_master` e `jobs_db` e vá em **Extensões → Apps Script**.
...
const CLIENT_SHEET = "clientes_db_master";
...
function handleJob(body) {
  ...
  const row = JOB_HEADERS.map((key) => mapJobValue(key, body, jobId));
  sheet.appendRow(row);
...
function mapJobValue(key, body, jobId) {
...
- `data_cadastro`, `origem_cadastro`, `trello_card_id` e `trello_card_url` são preenchidos pelo script/integrações.
- Reimplante o Web App sempre que editar o código.

## (Opcional) Normalizar cliente_id nos jobs antigos

Cole e execute manualmente no Apps Script para ajustar a aba `jobs_db` com base no `nome_fantasia` da `clientes_db_master`:
